...
public final class CommentTest {
    ...
    public static void run(TestRunner.TestContext ctx) {
        ...
        ctx.test("sort by text accepted", () -> {
            Helpers.Response r = Helpers.request("GET", BASE + "?sort=text&order=asc", null, Map.of());
            Assert.equal(200, r.status);
        });
        ...
        ctx.test("sort by numeric field \"upvoteCount\" orders numerically with missing last", () -> {
            java.util.List<String> ids = new java.util.ArrayList<>();
            for (int v : new int[]{100, 0, 9, 99}) {
                Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
                payload.put("upvoteCount", v);
                Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
                Assert.equal(201, r.status, "expected 201");
                @SuppressWarnings("unchecked") Map<String, Object> b = (Map<String, Object>) r.body;
                ids.add((String) b.get("id"));
            }
            // One item that omits the numeric field entirely.
            Helpers.Response r0 = Helpers.request("POST", BASE, Helpers.buildPayload(ENTITY, false), Map.of());
            Assert.equal(201, r0.status);
            @SuppressWarnings("unchecked") Map<String, Object> b0 = (Map<String, Object>) r0.body;
            String missingId = (String) b0.get("id");
            ids.add(missingId);

            Helpers.Response res = Helpers.request("GET", BASE + "?sort=upvoteCount&order=asc&limit=100", null, Map.of());
            Assert.equal(200, res.status);
            @SuppressWarnings("unchecked") Map<String, Object> rb = (Map<String, Object>) res.body;
            @SuppressWarnings("unchecked") List<Object> items = (List<Object>) rb.get("items");
            java.util.List<Map<String, Object>> mine = new java.util.ArrayList<>();
            for (Object o : items) {
                @SuppressWarnings("unchecked") Map<String, Object> m = (Map<String, Object>) o;
                if (ids.contains(m.get("id"))) mine.add(m);
            }
            java.util.List<Long> presentVals = new java.util.ArrayList<>();
            for (Map<String, Object> m : mine) {
                Object val = m.get("upvoteCount");
                if (val instanceof Number) presentVals.add(((Number) val).longValue());
            }
            Assert.equal(List.of(0L, 9L, 99L, 100L), presentVals);
            Assert.equal(missingId, mine.get(mine.size() - 1).get("id"));
        });


        ctx.test("filter on text field \"text\" returns matches", () -> {
            Helpers.Response c = Helpers.request("POST", BASE, Helpers.buildPayload(ENTITY, false), Map.of());
            @SuppressWarnings("unchecked") Map<String, Object> body = (Map<String, Object>) c.body;
            String value = (String) body.getOrDefault("text", "");
            ...
            Helpers.Response r = Helpers.request("GET", BASE + "?text=" + encoded, null, Map.of());
        ...
        ctx.test("leading/trailing whitespace is trimmed on create", () -> {
            Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
            payload.put("text", "  trimmed value  ");
            Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
            Assert.equal(201, r.status, "expected 201");
            @SuppressWarnings("unchecked") Map<String, Object> body = (Map<String, Object>) r.body;
            Assert.equal("trimmed value", body.get("text"));
        });

        ctx.test("control characters are stripped on create", () -> {
            Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
            payload.put("text", "clean\u0000\u0007ed");
            Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
            Assert.equal(201, r.status, "expected 201");
            @SuppressWarnings("unchecked") Map<String, Object> body = (Map<String, Object>) r.body;
            Assert.equal("cleaned", body.get("text"));
        });

        ctx.test("value over maxLength rejected with 400 VALIDATION_ERROR", () -> {
            Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
            payload.put("text", "a".repeat(10001));
            Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
            Assert.equal(400, r.status);
            @SuppressWarnings("unchecked") Map<String, Object> body = (Map<String, Object>) r.body;
        ...
        ctx.test("value at maxLength accepted", () -> {
            Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
            payload.put("text", "a".repeat(10000));
            Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
            Assert.equal(201, r.status, "expected 201");
        });

        ctx.test("multiline field \"text\" keeps internal newlines", () -> {
            Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
            payload.put("text", "first line\nsecond line");
            Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
            Assert.equal(201, r.status, "expected 201");
            @SuppressWarnings("unchecked") Map<String, Object> body = (Map<String, Object>) r.body;
            Assert.equal("first line\nsecond line", body.get("text"));
        });
