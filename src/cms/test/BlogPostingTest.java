...
public final class BlogPostingTest {
    ...
    public static void run(TestRunner.TestContext ctx) {
        ...
        ctx.test("sort by numeric field \"wordCount\" orders numerically with missing last", () -> {
            java.util.List<String> ids = new java.util.ArrayList<>();
            for (int v : new int[]{100, 0, 9, 99}) {
                Map<String, Object> payload = Helpers.buildPayload(ENTITY, false);
                payload.put("wordCount", v);
                Helpers.Response r = Helpers.request("POST", BASE, payload, Map.of());
                Assert.equal(201, r.status, "expected 201");
            ...
            Helpers.Response res = Helpers.request("GET", BASE + "?sort=wordCount&order=asc&limit=100", null, Map.of());
        ...
        ctx.test("GET by id embeds \"image\" as an array of objects", () -> {
            ...
            Helpers.Response g = Helpers.request("GET", BASE + "/" + created.get("id"), null, Map.of());
            @SuppressWarnings("unchecked") Map<String, Object> got = (Map<String, Object>) g.body;
            Object embedded = got.get("image");
            Assert.isTrue(embedded instanceof List, "GET by id embeds many-ref as an array");
            @SuppressWarnings("unchecked") List<Object> list = (List<Object>) embedded;
            Assert.isTrue(list.size() >= 1, "embedded array is non-empty");
            Assert.isTrue(list.get(0) instanceof Map, "embedded element is an object");
            @SuppressWarnings("unchecked") Map<String, Object> first = (Map<String, Object>) list.get(0);
            Assert.equal("ImageObject", first.get("@type"));
        });
